<p align="left">
  <img src="https://github.com/vsphere-tmm/Supervisor-Services/cert-manager/blob/main/cer-manager-logo.png" width="250" title="Velero Logo">
</p>

# Cert-Manager vSphere IaaS Service

Utlizing the opensource [cert-manager](https://cert-manager.io/docs/) project you can manage your certifcates in your Supervisor Clusters. The Cert-manager integration in WCP/Supervisor Cluster is fully supported by VMware when using the following issuers.  
*At somepoint provide Certificate delegation between the Supervisor Cluster and the Tanzu Kubernetes Clusters running within the Supervisor.

Supported Issuers:
CA & Selfsigned ClusterIssuer (Cluster Admin Only)
CA & Selfsigned Issuer
*DigiCert Issuer

## CA Issuer (Namespace Scoped)
Deploy a CA Issuer:
First create a certificate and private key (see XXX for examples on how to create a CA Certificate)
Encode the certifcate and private key using base64

Create a Kubernentes secret that contains the encoded certificate and private key
`
apiVersion: v1
kind: Secret
metadata:
  name: my-ca-key-pair
  namespace: my-namespace
data:
  tls.crt: 
  tls.key: 
`
Deploy the Secret
`kubectl apply -f ca-secret.yaml`

Deploy the CA Issuer

Request a Certficate:


## Cluster Scoped
*This can only be done by a VI Admin with access to the Supervisor in vCenter
Deploy a ClusterIssuer:


Request a Certficate:
*This can only be done by a VI Admin with access to the Supervisor in vCenter


*Supervisor Services are only available with Supervisor Clusters enabled using NSX-T*
  </br>
  </br>  
    
